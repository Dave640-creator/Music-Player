...
class MusicProvider extends ChangeNotifier {
  ...
  Future<void> generateAutoPlaylists() async {
    ...
    final configs = [
      {'name': '🎓 Study Mix', 'mood': 'focus', 'desc': 'Perfect for deep work'},
      {'name': '💪 Workout Mix', 'mood': 'workout', 'desc': 'High energy tracks'},
    ...
    for (final cfg in configs) {
      final exists = _playlists.any(
          (p) => p.name == cfg['name'] && p.isAutoGenerated);
      if (exists) continue;
  ...
  Future<void> _checkBadges() async {
    ...
    if (stats.totalSongsPlayed >= 100) await unlock(BadgeType.hundredSongsPlayed);
